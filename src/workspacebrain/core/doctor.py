...
class BrainDoctor:
    ...
    def _check_ai_rule_files(
        ...
        """Check AI rule files in project root and detect drift."""
        results = []

        # Check each AI rule file in project root
        for rule_file, ai_tool in AI_RULE_FILES.items():
    ...
    def _check_drift(
        ...
        ai_tool = AI_RULE_FILES.get(rule_file, "unknown")

        if current_hash == expected_hash:
            return CheckResult(
                name=rule_file,
                status=CheckStatus.OK,
                message=f"In sync ({ai_tool})",
            )
        else:
            return CheckResult(
                name=rule_file,
                status=CheckStatus.WARNING,
                message=f"Drift detected ({ai_tool})",
                details=[

...
class BrainDoctor:
    ...
    def _check_ai_rule_files(
        ...
        """Check AI rule files in .wbrain/ directory and detect drift."""
        results = []

        # Check .wbrain directory exists
        wbrain_dir = project_path / AI_RULES_DIR
        if not wbrain_dir.exists():
            results.append(
                CheckResult(
                    name=f"{AI_RULES_DIR}/",
                    status=CheckStatus.WARNING,
    ...
    def _check_drift(
        ...
        display_name = f"{AI_RULES_DIR}/{rule_file}"
